...
func _test_scene_and_ui() -> void:
	...
	_record("reference-scene-ui", root is Control and status is Label and progress is ProgressBar and play_button is Button and reset_button is Button, "The playable reference scene and its complete UI instantiate.")
	if root != null:
		root.free()
