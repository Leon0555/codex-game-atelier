...
Phase 1 follows a three-milestone plan. M1 is complete locally on macOS Apple Silicon: the Go CLI closes a special-path `detect → doctor → initialize → validate → test → Debug/Release export → target smoke` flow, and the deterministic Starter Template contains the playable six-test Atelier Spark vertical slice. M2 has implemented logical capability Profiles, a file-recovered native collaboration trace with separate read-only audit and repair, the monotonic gate-policy contract, build/export policy consumption, read-only three-mode `release check`, an explicitly managed optional pre-commit hook, and a required hosted macOS Apple Silicon CI check on the minimum Go 1.24 toolchain. M3 rejected rc.2 through rc.5 after independent audit or real client warnings, then produced reproducible `0.3.0-rc.6`. Its isolated remote Plugin installation, real lifecycle, Skill discovery, Godot E2E, required CI, bound strict check and independent final read-only audit are verified; strict reports 12/12 PASS and the audit closed at 0 Blocker/High/Medium/Low. ADR 0027 accepts this single-machine isolated validation as the v1 clean-environment evidence; it does not claim multi-user or multi-machine validation. The public repository is `Leon0555/codex-game-atelier`, and `main` requires `verify-macos-arm64`. A protected version ref and explicit user release approval remain open. There is still no published v1.0 package.
...
Start with [docs/project-brief.md](docs/project-brief.md), [docs/architecture.md](docs/architecture.md), [docs/support-matrix.md](docs/support-matrix.md), and [docs/v1-acceptance.md](docs/v1-acceptance.md). ADR 0004 freezes Go as the v1 CLI production language; ADRs 0005-0027 define state/evidence, initial commands, Godot workflows, reproducible packaging, release gates, Plugin-only v1 distribution, embedded Starter creation, the macOS-only v1 support scope, bound external release evidence, and single-machine isolated final validation.
...
See [SECURITY.md](SECURITY.md). v1.0 is not published and Private Vulnerability Reporting must still be enabled and verified before release; do not post sensitive vulnerability details in a public issue.
