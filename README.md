...
Phase 1 follows a three-milestone plan. M1 is complete locally on macOS Apple Silicon: the Go CLI closes a special-path `detect → doctor → initialize → validate → test → Debug/Release export → target smoke` flow, and the deterministic Starter Template contains the playable six-test Atelier Spark vertical slice. M2 has implemented logical capability Profiles, a file-recovered native collaboration trace with separate read-only audit and repair, the monotonic gate-policy contract, build/export policy consumption, read-only three-mode `release check`, an explicitly managed optional pre-commit hook, and a required hosted macOS Apple Silicon CI check on the minimum Go 1.24 toolchain. M3 rejected `0.3.0-rc.2`, fixed its host-support and external-evidence defects, and produced a reproducible `0.3.0-rc.3`; its remote Plugin, Godot E2E, lifecycle, CI and strict 12/12 evidence passed, but final audit found that several public commands could still execute on unsupported Windows/Linux hosts. rc.3 is therefore rejected; a global pre-dispatch host gate and regression test are implemented in the next source change, which must become a fresh rc.4. The public repository is `Leon0555/codex-game-atelier`, and `main` requires `verify-macos-arm64`. A separate clean-user/machine repetition, protected version ref and explicit user release approval remain open. There is still no published v1.0 package.
