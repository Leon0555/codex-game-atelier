...
Phase 1 follows a three-milestone plan. M1 is complete locally on macOS Apple Silicon: the Go CLI closes a special-path `detect → doctor → initialize → validate → test → Debug/Release export → target smoke` flow, and the deterministic Starter Template contains the playable six-test Atelier Spark vertical slice. M2 has implemented logical capability Profiles, a file-recovered native collaboration trace with separate read-only audit and repair, the monotonic gate-policy contract, build/export policy consumption, read-only three-mode `release check`, an explicitly managed optional pre-commit hook, and a hosted macOS Apple Silicon CI run that passes on the minimum Go 1.24 toolchain. M3 has a reproducible `0.3.0-rc.1` single-Plugin local candidate from clean revision `969bef0`; the Git-backed remote Plugin install, package identity, special-path Starter/Headless/6-test flow, real user-level `rc.0 → rc.1` upgrade, rejected invalid candidate, `rc.1 → rc.0` rollback, uninstall, and exact user-config restoration have passed. The public source repository now exists at `Leon0555/codex-game-atelier`. Remote new-task Skill discovery, required-check branch protection, protected release provenance, a separate clean-user/machine repetition, and final independent audit remain open. There is still no v1.0-ready or published package.
...
Start with [docs/project-brief.md](docs/project-brief.md), [docs/architecture.md](docs/architecture.md), [docs/support-matrix.md](docs/support-matrix.md), and [docs/v1-acceptance.md](docs/v1-acceptance.md). ADR 0004 freezes Go as the v1 CLI production language; ADRs 0005-0025 define state/evidence, initial commands, Godot workflows, reproducible packaging, release gates, Plugin-only v1 distribution, embedded Starter creation, and the macOS-only v1 support scope.
