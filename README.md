...
Phase 1 follows a three-milestone plan. M1 is complete locally on macOS Apple Silicon: the Go CLI closes a special-path `detect → doctor → initialize → validate → test → Debug/Release export → target smoke` flow, and the deterministic Starter Template contains the playable six-test Atelier Spark vertical slice. M2 has implemented logical capability Profiles, a file-recovered native collaboration trace with separate read-only audit and repair, the monotonic gate-policy contract, build/export policy consumption, read-only three-mode `release check`, an explicitly managed optional pre-commit hook, and a required hosted macOS Apple Silicon CI check on the minimum Go 1.24 toolchain. M3 rejected rc.2 through rc.5 after independent audit or real client warnings, then used the fully audited `0.3.0-rc.6` as the upgrade and rollback baseline for `1.0.0`. The stable candidate has passed reproducible packaging, isolated remote Plugin installation, a real user-level lifecycle with exact state restoration, new-task Skill discovery, special-path Godot E2E, required CI, and the bound strict check at 12/12 PASS. Its independent final read-only audit remains required before publication. ADR 0027 accepts this single-machine isolated validation as the v1 clean-environment evidence; it does not claim multi-user or multi-machine validation. ADR 0028 freezes the first public stable version as `1.0.0`; rc.6 artifacts were not relabeled. The public repository is `Leon0555/codex-game-atelier`, and `main` requires `verify-macos-arm64`. A protected version ref and explicit external-publication approval remain open. There is still no published v1.0 package.
