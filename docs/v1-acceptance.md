...
| V1-07 零构建分发入口 | M3 | 普通用户无需 clone/npm build；已有 Godot 时最多三个主要步骤；Plugin 可安装/加载/调用，Starter Template 可取得/初始化，CLI 为预构建产物 | 干净用户路径及步骤计数、包内容、实际调用 | PARTIAL（同版本本地 candidate、当前 Codex 格式的专用 marketplace A/B、两个 archive/checksum、许可证与 Apple Silicon trusted 入口已 PASS；真实 Codex 安装/发现/调用仍 NOT RUN） |
| V1-08 生命周期与供应链 | M3 | 安装、升级、卸载、回滚保留用户项目和凭据；无默认遥测/隐藏网络或外部写入；发布物包含 checksum、来源和许可；npm 方案使用 Trusted Publishing/2FA/provenance | 生命周期前后 diff、网络/文件审计、artifact manifest、预发布配置审计 | PARTIAL（分发 manifest、精确版本闭合、MIT/NOTICE、checksum 与本地重现性 PASS；生命周期、动态网络/文件审计和 npm 预发布配置仍 NOT RUN） |
