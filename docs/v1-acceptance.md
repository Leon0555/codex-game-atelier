...
| V1-06 模式、Hooks 与 CI | M2 | `manual` 保留安全门禁，`standard` 自动执行生产子集，`strict` 聚合发布条件；hook 只能显式安装并可卸载；无 hook/`--no-verify` 不能绕过 CLI/CI | 三模式正反例、hook 路径 diff、CI workflow 审计 | PARTIAL（版本化门禁表、Schema、单调/mandatory 负例与 Plugin 分发已 PASS；CLI 消费、hook 和 CI 未实现） |
