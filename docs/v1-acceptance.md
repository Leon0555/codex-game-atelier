...
| V1-08 生命周期与供应链 | M3 | 安装、升级、卸载、回滚保留用户项目和凭据；无默认遥测/隐藏网络或外部写入；发布物包含 checksum、来源和许可；npm 方案使用 Trusted Publishing/2FA/provenance | 生命周期前后 diff、网络/文件审计、artifact manifest、预发布配置审计 | PARTIAL（分发 manifest、精确版本闭合、MIT/NOTICE、checksum、本地重现性及真实安装/卸载清理 PASS；升级、失败升级、回滚、动态网络/文件审计和 npm 预发布配置仍 NOT RUN） |
