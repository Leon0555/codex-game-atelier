...
| V1-07 零构建分发入口 | M3 | 普通用户无需 clone/npm build；已有 Godot 时最多三个主要步骤；Plugin 可安装/加载/调用，Starter Template 可取得/初始化，CLI 为预构建产物 | 干净用户路径及步骤计数、包内容、实际调用 | PARTIAL（同版本本地 candidate、两个 archive/checksum、许可证与 Apple Silicon trusted 入口已 PASS；真实 Codex 安装/发现/调用仍 NOT RUN） |
