...
| V1-06 模式、Hooks 与 CI | M2 | `manual` 保留安全门禁，`standard` 自动执行生产子集，`strict` 聚合发布条件；hook 只能显式安装并可卸载；无 hook/`--no-verify` 不能绕过 CLI/CI | 三模式正反例、hook 路径 diff、CI workflow 审计 | PARTIAL（策略、CLI 消费、只读 release check、显式 hook 生命周期与最小 macOS CI contract 已在本地 PASS；strict M3 gates 与首次 GitHub-hosted CI 仍 NOT RUN） |
| V1-07 零构建分发入口 | M3 | 普通用户无需 clone/npm build；已有 Godot 时最多三个主要步骤；Plugin 可安装/加载/调用，Starter Template 可取得/初始化，CLI 为预构建产物 | 干净用户路径及步骤计数、包内容、实际调用 | PARTIAL（同版本本地 candidate、当前 Codex 格式的专用 marketplace A/B、两个 archive/checksum、许可证与 Apple Silicon trusted 入口已 PASS；真实 Codex 安装、新任务 Skill 发现和包内 CLI 调用已 PASS；干净用户路径及 Starter 初始化仍 NOT RUN） |
| V1-08 生命周期与供应链 | M3 | 安装、升级、卸载、回滚保留用户项目和凭据；无默认遥测/隐藏网络或外部写入；发布物包含 checksum、来源和许可；npm 方案使用 Trusted Publishing/2FA/provenance | 生命周期前后 diff、网络/文件审计、artifact manifest、预发布配置审计 | PARTIAL（archive 安全、checksum、本地重现性、静态秘密/网络/遥测/写入边界及真实安装/卸载清理 PASS；只读审计发现 dirty-source provenance、缺少 Go notice 和 framework Gatekeeper 三项 release blocker；升级/回滚、动态 trace、remote provenance 仍 NOT RUN） |
