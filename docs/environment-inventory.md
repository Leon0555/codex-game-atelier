...
| Godot | 项目本地 `.tools/godot/4.7.2` 已安装 `4.7.2.stable.official.ed1daf0bf` 标准版和匹配模板 | 否 | Phase 1 Godot adapter/headless/export 的核心外部依赖 | 官方包散列、版本、Notarized Developer ID、headless 与技术导出已实测；标准 `user://` 现由显式授权与 intent external-write 声明控制 |
...
| Go | `.tools/go/1.27.0`，`go1.27.0 darwin/arm64` | 否 | v1 CLI 生产实现工具链 | 官方 SHA-256 通过；`detect`/`doctor`/`initialize`/`status`/`validate` 与 Godot Headless 薄切片已在正式包路径实现，完整 CLI 与目标宿主验证仍未完成 |
...
- 未安装 CMake 或容器运行时。
- Godot Headless 的“零仓库外写入”目标已被 ADR 0009 取代：未经授权保持 BLOCKED；明确授权后允许并记录官方标准 `user://`。其他隐藏外部写入仍不允许。
