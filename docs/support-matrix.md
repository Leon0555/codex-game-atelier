...
生产级不是“Godot 官方可以下载”，而是：对每个列出的 `框架版本 + Godot patch + 宿主 + 能力/导出目标` 元组，有可复现 PASS evidence、稳定错误与恢复路径、隔离式干净环境复验、安装/升级/卸载验证，以及独立只读发布审计。v1 的干净环境可以在同一 Apple Silicon Mac 上由固定 clean revision、全新隔离 `CODEX_HOME`、远程固定 ref、安装 cache 与候选逐文件一致、全新项目路径和真实用户级状态恢复共同证明；不要求第二 UID 或第二台实体机器，也不据此声明多用户/多机器已验证。缺失的元组必须标成 BLOCKED/NOT RUN/不支持。
