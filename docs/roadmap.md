...
- 干净 Starter Template 及其独立 archive/checksum 历史验证；正式 `1.0.0` 已将其作为 Plugin 内含能力，不再单独面向用户发布。

这些结果与 M3 的远程安装、生命周期、严格门禁和独立终审证据共同关闭了 v1.0 发布门禁。

## 2. 已完成的三里程碑实施路线

Phase 1 没有按“核心、适配器、参考游戏、分发、发布”串行铺开，而是按三个可运行的垂直里程碑完成；每个里程碑均留下了实际命令、产物和 evidence。
...
- 只处理冻结范围内有证据的缺陷、安全修复、文档一致性和 CI 健康问题。
- 监测 Godot 4.7.x 上游 patch；新 patch 不自动进入 Support Matrix，触发时按 7 个自然日目标完成候选复验。
- 没有新的范围决策时，不启动 Phase 2，不加入新引擎、宿主、导出目标、运行时或分发渠道。
- 任何新的正式 tag、Marketplace 更新或其他外部发布仍需用户明确授权。
